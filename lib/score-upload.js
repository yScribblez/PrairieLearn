...
module.exports = {
    ...
    _updateInstanceQuestionFromJson(json, assessment_id, authn_user_id, callback) {
        ...
        if (!_.has(json, 'qid')) return callback(new Error('"qid" not found'));
        ...
        sqldb.queryOneRow(sql.select_instance_question, params, function(err, result) {
            if (err) return callback(new Error(`unable to locate question ${json.qid} in instance ${json.instance} for ${json.uid}: ${String(err)}`));
            const instance_question_id = result.rows[0].instance_question_id;
