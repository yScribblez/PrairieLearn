...
module.exports.gradingJobStatusUpdated = function(grading_job_id) {
    ...
    sqldb.queryOneRow(sql.select_submission_for_grading_job, params, (err, result) => {
        ...
        this.namespace.to(`variant-${result.rows[0].variant_id}`).emit('submission:status', data);
    });
};

module.exports.variantSubmissionCreated = function(variant_id, submission_id) {
    module.exports.getVariantSubmissionsStatus(variant_id, (err, submissions) => {
        if (ERR(err, (err) => logger.error(err))) return;

        // Select only this particular submission
        const submission = submissions.find(s => s.id == submission_id);
        const data = {
            variant_id,
            submission,
        };
        this.namespace.to(`variant-${variant_id}`).emit('submission:new', data);
    });
};
