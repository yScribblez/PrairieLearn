...
class Grader {
    handleGradingRequest(grading_job, submission, variant, question, course) {
        ...
        if (process.env.DOCKER_PORT) {
            // Connect to Docker on localhost at the given port over http(s)
            docker = new Docker({
                host: '127.0.0.1',
                port: process.env.DOCKER_PORT,
            });
        } else {
            // Assume we're running with a socket, either via a mount or locally
            // Using a socket is the Dockerode default
            docker = new Docker();
        }
