...
const { generateAuthToken } = require('./questionFiles');
...
module.exports = {
    ...
    */
     ...
     */
    _makeAndInsertVariantWithClient(client, question_id, instance_question_id, user_id, authn_user_id, course_instance_id, course, options, callback) {
     ...
     * @param {function} callback - A callback(err, variant) function.
     */
    _ensureVariantWithClient(client, question_id, instance_question_id, user_id, authn_user_id, course_instance_id, course, options, require_open, callback) {
        if (instance_question_id != null) {
            ...
            sqldb.callWithClient(client, 'instance_questions_select_variant', params, (err, result) => {
                ...
                this._makeAndInsertVariantWithClient(client, question_id, instance_question_id, user_id, authn_user_id, course_instance_id, course, options, (err, variant) => {
                    if (ERR(err, callback)) return;
     ...
     * @param {function} callback - A callback(err, variant) function.
     */
    ensureVariant(question_id, instance_question_id, user_id, authn_user_id, course_instance_id, course, options, require_open, callback) {
        let variant;
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                (callback) => {
                    this._ensureVariantWithClient(client, question_id, instance_question_id, user_id, authn_user_id, course_instance_id, course, options, require_open, (err, ret_variant) => {
                        if (ERR(err, callback)) return;
                        variant = ret_variant;
                        callback(null);
                    });
     ...
     * @param {function} callback - A callback(err, variant) function.
     */
    _testQuestion(question, course_instance, course, authn_user_id, callback) {
        debug('_testQuestion()');
     ...
     */
    _runTest(logger, showDetails, question, course_instance, course, authn_user_id, callback) {
        let variant, expected_submission, test_submission, success = true;
        async.series([
            (callback) => {
                logger.verbose('Testing ' + question.qid);
                this._testQuestion(question, course_instance, course, authn_user_id, (err, ret_variant, ret_expected_submission, ret_test_submission) => {
                    if (ERR(err, callback)) return;
                    variant = ret_variant;
                    expected_submission = ret_expected_submission;
                    test_submission = ret_test_submission;
     ...
     */
    startTestQuestion(count, showDetails, question, course_instance, course, authn_user_id, callback) {
        let success = true;
        const options = {
            course_id: course.id,
        ...
        serverJobs.createJobSequence(options, (err, job_sequence_id) => {
            ...
            serverJobs.createJob(jobOptions, (err, job) => {
                ...
                async.eachSeries(_.range(count), (iter, callback) => {
                    ...
                    this._runTest(job, showDetails, question, course_instance, course, authn_user_id, (err, ret_success) => {
                        if (ERR(err, callback)) return;
    ...
    _render(renderSelection, variant, question, submission, submissions, course, course_instance, locals, callback) {
        questionServers.getModule(question.type, (err, questionModule) => {
            ...
            questionModule.render(renderSelection, variant, question, submission, submissions, course, course_instance, locals, (err, courseIssues, htmls) => {
                ...
                this._writeCourseIssues(null, courseIssues, variant.id, user_id, studentMessage, courseData, (err) => {
                    if (ERR(err, callback)) return;
     ...
     */
    _buildQuestionUrls(urlPrefix, variant, question, instance_question, course_instance, assessment) {
        const urls = {};
        ...
        if (!assessment) {
            ...
            const token = generateAuthToken(variant.authn_user_id, variant.id);
            urls.filesApiUrl = `/pl/auth/${variant.authn_user_id}/${token}/instructor/${course_instance.id}/${variant.id}/files`;
            ...
            const token = generateAuthToken(variant.authn_user_id, variant.id);
            urls.filesApiUrl = `/pl/auth/${variant.authn_user_id}/${token}/${variant.id}/files`;
        }
        ...
        async.series([
            (callback) => {
                if (variant_id != null) {
                    ...
                    this.ensureVariant(locals.question.id, instance_question_id, locals.user.user_id, locals.authn_user.user_id, course_instance_id, locals.course, {}, require_open, (err, variant) => {
                        if (ERR(err, callback)) return;
            ...
            (callback) => {
                ...
                const urls = this._buildQuestionUrls(urlPrefix, variant, question, instance_question, course_instance, assessment);
    ...
    renderPanelsForSubmission(submission_id, urlPrefix, questionContext, csrfToken, callback) {
        ...
        sqldb.queryOneRow(sql.select_submission_info, params, (err, results) => {
            ...
            // Fake locals. Yay!
            const locals = this._buildQuestionUrls(urlPrefix, variant, question, instance_question, course_instance, assessment);
